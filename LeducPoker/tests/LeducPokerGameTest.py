...
class IsTerminalTest(unittest.TestCase):
    def test_game_start_is_not_terminal(self):
        bet_sequence = ()
        self.assertFalse(LeducPokerGame.LeducNode([bet_sequence, ()], hidden_board_card=0).is_terminal)

    def test_p1_folds_is_terminal(self):
        bet_sequence = (PlayerActions.BET_RAISE, PlayerActions.FOLD)
        self.assertTrue(LeducPokerGame.LeducNode([bet_sequence, ()], hidden_board_card=0).is_terminal)
    ...
    def test_round1_is_not_terminal(self):
        bet_sequences = [
            (PlayerActions.CHECK_CALL, PlayerActions.CHECK_CALL),
            ()]
        self.assertFalse(LeducPokerGame.LeducNode(bet_sequences, hidden_board_card=1).is_terminal)


class PayoffsTest(unittest.TestCase):
    def test_nonterminal_payoff_raises_exception(self):
        with self.assertRaises(RuntimeError):
            LeducPokerGame.LeducNode(bet_sequences=[(),()], hidden_board_card=0).get_payoffs([0, 0])
    ...
    def test_bet_fold(self):
        ...
        payoffs = LeducPokerGame.LeducNode(bet_sequences=bet_sequences, hidden_board_card=0).get_payoffs([1, 2])
        self.assertEqual([1, -1], payoffs.tolist())

        bet_sequences = [(PlayerActions.CHECK_CALL, PlayerActions.BET_RAISE, PlayerActions.FOLD), ()]
        payoffs = LeducPokerGame.LeducNode(bet_sequences=bet_sequences, hidden_board_card=0).get_payoffs([2, 1])
        self.assertEqual([-1, 1], payoffs.tolist())
