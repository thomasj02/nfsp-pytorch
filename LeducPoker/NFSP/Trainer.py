from LeducPoker.NFSP.Agent import NfspAgent, collect_trajectories
...
if __name__ == "__main__":
    ...
    _q_policy_parameters = QPolicyParameters(
        ...
        learning_rate=1e-1)
    _q_network_local = QNetwork(state_size=22, action_size=3).to(device)
    _q_network_target = QNetwork(state_size=22, action_size=3).to(device)
    ...
    _supervised_network = SupervisedNetwork(state_size=22, action_size=3).to(device)
    ...
    print("Init exploitability:", Exploitability.get_exploitability(_nfsp_agents[0].leduc_supervised_policy))
    print("Init exploitability:", Exploitability.get_exploitability(_nfsp_agents[1].leduc_supervised_policy))
    ...
    with tqdm(range(_episodes)) as t:
        for e in t:
            ...
            with torch.no_grad():
                ...
                exploitability = Exploitability.get_exploitability(_nfsp_agents[0].leduc_supervised_policy)
                ...
                #log_strategy(_writer, _nfsp_agents[0].Leduc_supervised_policy, e)
